...
export default function TimelineEntry({
  ...
  const cardProps = {
    ...
    entry, onOpenLightbox, isPublic, reactions, onReact, hasReacted,
  }

  return (
    <div ref={ref} className={`${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
      {/* Desktop zigzag — cards closer to center (mr-4/ml-4 instead of mr-8/ml-8) */}
      <div className="hidden md:flex items-start">
        <div className="flex-1">
          {isLeft ? <div className="mr-4"><Card {...cardProps} /></div> : <div />}
        </div>
        ...
        <div className="flex-1">
          {!isLeft ? <div className="ml-4"><Card {...cardProps} /></div> : <div />}
        </div>
      </div>
...
function Dot({ dotColor, dotGlow, completed, is_fail }) {
  return (
    <div className={`relative w-6 h-6 rounded-full ${dotColor} ${dotGlow} border-2 border-white/20 flex-shrink-0`}>
      {completed && !is_fail && (
        <svg className="w-3.5 h-3.5 text-white absolute top-[3px] left-[3px]" fill="currentColor" viewBox="0 0 20 20">
...
function Card({
  ...
  return (
    <div
      className={`rounded-2xl p-4 sm:p-5 card-hover relative overflow-hidden mb-6`}
      style={{
        background: 'rgba(255, 255, 255, 0.05)',
        backdropFilter: 'blur(20px)',
        WebkitBackdropFilter: 'blur(20px)',
        border: is_fail ? '1px solid rgba(249, 115, 22, 0.25)' : '1px solid rgba(255, 255, 255, 0.08)',
        boxShadow: is_fail ? '0 0 20px rgba(249, 115, 22, 0.15)' : 'none',
      }}
    >
      {/* Day watermark */}
      ...
      {media_type === 'image' && media_url && (
        ...
        >
          <img
            src={media_url}
            alt={`Ngày ${day}`}
            className="w-full h-52 sm:h-60 object-cover transition-all duration-500 group-hover:scale-105 group-hover:brightness-110"
            loading="lazy"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          {/* Glassmorphism mood badge overlay */}
          <div
      ...
      {media_type === 'video' && media_url && (
        ...
        >
          <video src={media_url} className="w-full h-52 sm:h-60 object-cover" muted preload="metadata" />
      ...
      {/* Mood badge — inline when no media */}
      {(media_type === 'none' || !media_url) && (
        <div className="mb-3"><MoodBadge mood={mood} /></div>
      )}

      {/* Caption — better line-height */}
      <p
        className={`text-sm relative z-10 ${is_fail ? 'italic text-orange-300/90' : 'text-white/75'}`}
